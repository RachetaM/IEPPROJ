...
int PWM :: getFactor(){
    return freq*100/factor;
}

PWM ::PWM(int freq, int factor){
    this->freq=freq;
    //this->factor=factor;
    this->factor= freq*factor/100;
    setFreq(freq);
}

void PWM :: setFactor(int factorsec,uint8_t channel){
    int scalat=0;
...
void PWM :: setFreq(int frecv){
    ...
    bcm2835_peri_write(bcm2835_clk + BCM2835_PWMCLK_DIV, BCM2835_PWM_PASSWRD | (frecv << 12));
    bcm2835_peri_write(bcm2835_clk + BCM2835_PWMCLK_CNTL, BCM2835_PWM_PASSWRD | 0x11); /* Source=osc and enable */
}

void PWM :: setRange(uint8_t channel){
    if (   bcm2835_clk == MAP_FAILED
         || bcm2835_pwm == MAP_FAILED)
      return; /* bcm2835_init() failed or not root */
